...
<xsl:stylesheet version="2.0"
    ...
    <xsl:template match="//ds:subsystem/ds:datasources/ds:datasource[@jndi-name='java:jboss/datasources/KeycloakDS']">
        <ds:datasource jndi-name="java:jboss/datasources/KeycloakDS" enabled="true" use-java-context="true" pool-name="KeycloakDS" use-ccm="true">
            <ds:connection-url>jdbc:postgresql://${env.POSTGRES_PORT_5432_TCP_ADDR}:${env.POSTGRES_PORT_5432_TCP_PORT}/${env.KEYCLOAK_DATABASE:keycloak}</ds:connection-url>
            <ds:driver>postgresql</ds:driver>
            <ds:security>
              <ds:user-name>${env.KEYCLOAK_USERNAME:keycloak}</ds:user-name>
              <ds:password>${env.KEYCLOAK_PASSWORD:password}</ds:password>
            </ds:security>
            <ds:validation>
